...
def run_analysis():
    ...
    # We will process them exactly like execute_swing_scan, but keep everything
    stats_data = []
    for r in raw_results:
        decision_str = getattr(r, "decision", "WATCH")
        
        # In execute_swing_scan, it only processes BUY, STRONG_BUY, SELL, STRONG_SELL, WATCH
        ...
        # Emulate process_post_scan values
        ...
        # Evaluate Rejections (simulate the block at line ~377)
        rejected_by = []
        is_borderline = False
        notes = []
        ...
        if score < 50.0 and confidence < 50.0:
            ...
            mode = getattr(service.config, 'swing_signal_mode', 'Balanced')
            min_score = 75.0
            min_conf = 70.0
            min_rr = 1.8
            
            if confidence < min_conf:
                rejected_by.append("Confidence")
            if score < min_score:
                rejected_by.append("Score")
            
            # SPRINT-80 checks
            # In execute_swing_scan, Elite Selection drops execution_status != READY
            # In ranking/score_engine.py, MTF Major Conflict changes decision to WAIT
            # And ADX Sideways changes decision to WATCH
            # Let's see what the final decision was in the real pipeline
